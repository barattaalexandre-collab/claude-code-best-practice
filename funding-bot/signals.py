...
def generate_signal(symbol: str, funding: float, oi: float, prices: list[float]) -> Signal | None:
    ...
    if ENABLE_LONG_SIGNALS and funding < FUNDING_LONG_THRESHOLD and _is_stable_or_rising(prices):
        return {
            "symbol": symbol,
            "type": "LONG",
            "funding": funding,
    ...
    if ENABLE_SHORT_SIGNALS and funding > FUNDING_SHORT_THRESHOLD and _is_not_parabolic(prices):
        return {
            "symbol": symbol,
            "type": "SHORT",
            "funding": funding,
